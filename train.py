...
def train_with_accuracy_focus(args, model, optimizer, loss_fn, g, features, labels, test_mask, device):
    start_epoch = 0
    best_f1 = 0
    best_epoch = 0
    patience_counter = 0
    ...
    for epoch in range(start_epoch, args.n_epochs):
        ...
        # Training
        model.train()
        pred = model(g, features.to(device))
        ...
        model.eval()
        with th.no_grad():
            current_f1 = evaluate(model, g, features, labels, device)
        
        print("Epoch {:05d}, Time(s) {:.4f}, Loss {:.4f}, F1 {:.4f}".format(
            epoch, np.mean(duration), loss_val, current_f1))
    ...
    # Return the best model, or the current model if no best was found
    return best_model if best_model is not None else model

# Also modify the main section to ensure output directory exists:
...
if __name__ == '__main__':
    ...
    best_model = train_with_accuracy_focus(args, model, optim, loss, g, features, labels, test_mask, device)
    
    print("Getting final predictions")
    class_preds, pred_proba = get_model_class_predictions(best_model, g, features, labels, device, args.threshold)
